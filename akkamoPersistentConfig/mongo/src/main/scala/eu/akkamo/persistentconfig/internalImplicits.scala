...
private[persistentconfig] object internalImplicits {
  ...
  implicit object StringVDV extends VDV[String] {
    override def apply(t: String): Document = Document("v" -> t)

    override val back = (d: Document) => d.get[BsonString]("v").map(_.getValue)
  }

  implicit object StringListVDV extends ListVDV[String] {
    override def apply(t: List[String]): Document = Document("v" -> t)

    override def convert = (p: BsonValue) => p.asString().getValue
  }

  implicit object IntVDV extends VDV[Int] {
    override def apply(t: Int): Document = Document("v" -> t)

    override val back = (d: Document) => d.get[BsonInt32]("v").map(_.getValue)
  }

  implicit object IntListVDV extends ListVDV[Int] {
    override def apply(t: List[Int]): Document = Document("v" -> t)

    override def convert = (p: BsonValue) => p.asInt32().getValue
  }

  implicit object LongVDV extends VDV[Long] {
    override def apply(t: Long): Document = Document("v" -> t)

    override val back = (d: Document) => d.get[BsonInt64]("v").map(_.getValue)
  }

  implicit object LongListVDV extends ListVDV[Long] {
    override def apply(t: List[Long]): Document = Document("v" -> t)

    override def convert = (p: BsonValue) => p.asInt64().getValue
  }

  implicit object DoubleVDV extends VDV[Double] {
    override def apply(t: Double): Document = Document("v" -> t)

    override val back = (d: Document) => d.get[BsonDouble]("v").map(_.getValue)
  }

  implicit object DoubleListVDV extends ListVDV[Double] {
    override def apply(t: List[Double]): Document = Document("v" -> t)
  ...
  implicit object BooleanVDV extends VDV[Boolean] {
    override def apply(t: Boolean): Document = Document("v" -> t)

    override val back = (d: Document) => d.get[BsonBoolean]("v").map(_.getValue)
  }
